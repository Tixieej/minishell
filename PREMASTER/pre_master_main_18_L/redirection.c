...
/*   Updated: 2021/03/11 13:53:53 by livlamin      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
int	in_red(t_command *command, t_list *begin)
{
	...
	while ((*cur_struct)->in_red)
	{
		...
		if (command->fd_in < 0)
		{
			command_not_found(command, "No such file or directory",
				(char *)(*cur_struct)->in_red->content, 127);
			return (-1);
		}
...
int	out_red(t_command *command)
{
	...
	while ((*cur_struct)->out_red)
	{
		...
		{
			command_not_found(command, "No such file or directory",
				(char *)(*cur_struct)->out_red->content, 127);
			return (-1);
		}
